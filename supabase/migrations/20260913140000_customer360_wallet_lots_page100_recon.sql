-- Customer 360 point lots: default page size 100 + wallet vs lot reconciliation summary.

CREATE OR REPLACE FUNCTION public.bff_admin_get_member_wallet_lot_reconciliation(p_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT COALESCE(uw.points_balance, 0)::integer
  ...
  SELECT COALESCE(SUM(wl.deductible_balance), 0)
  INTO v_lot_total
  ...
  RETURN jsonb_build_object(
    ...
    'lot_deductible_total', COALESCE(v_lot_total, 0),
    'lot_reconciliation_diff', v_points_balance::numeric - COALESCE(v_lot_total, 0)
  );
END;
$function$;

GRANT EXECUTE ON FUNCTION public.bff_admin_get_member_wallet_lot_reconciliation(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bff_admin_get_member_wallet_lot_reconciliation(uuid) TO service_role;
