-- Loyalty cache refresh scheduler — plan LOYALTY_PROGRESS_EXPIRY_CACHE_PLAN.md rev. 6.
--
-- Fixes vs 20260902104500_loyalty_cache_daily_catchup.sql:
-- 1. Pass B (daily catch-up) runs at most once per Bangkok calendar day per merchant
--    (or immediately on invalidate), not every 5-minute tick.
-- 2. When pass B runs, drain the full backlog in one fn_loyalty_cache_refresh_5m call
--    (loop internal 500-user chunks until has_more = false).
-- 3. internal_chunk_size is cursor step size only; not a daily user cap (apply p_batch_size
...
BEGIN
  SELECT sc.last_finished_at, COALESCE((sc.cursor->>'invalidate')::boolean, false)
  INTO v_last_finished_at, v_invalidate
...
BEGIN
  ...
  VALUES (
    'loyalty_cache_daily_catchup',
    p_merchant_id,
    'running',
    jsonb_build_object('invalidate', true, 'after_user_id', NULL),
    now()
  )
  ON CONFLICT (job_name, merchant_id) DO UPDATE SET
    status = 'running',
    cursor = jsonb_build_object('invalidate', true, 'after_user_id', NULL),
    updated_at = now();

  RETURN jsonb_build_object('ok', true, 'merchant_id', p_merchant_id, 'invalidate', true);
END;
$function$;
...
DECLARE
  ...
  v_max_catchup_iterations constant integer := 10000;
BEGIN
  ...
  BEGIN
    ...
    LOOP
      ...
      BEGIN
        -- Pass A: 5-minute dirty refresh (all dirty users; internal sub-chunks only).
        ...
        -- Pass B: daily catch-up when due (at most once per Bangkok day unless invalidate).
        IF public.fn_loyalty_cache_catchup_due(v_merchant.merchant_id) THEN
          ...
          LOOP
            ...
            IF v_iter >= v_max_catchup_iterations THEN
              RAISE EXCEPTION
                'loyalty cache catch-up exceeded max_iterations (%) for merchant %',
                v_max_catchup_iterations,
                v_merchant.merchant_id;
            END IF;
          END LOOP;
          ...
          v_catchup_pass := jsonb_build_object(
            ...
            'backlog_complete', COALESCE((v_catchup->>'backlog_complete')::boolean, true)
          );
        ELSE
          v_catchup_pass := jsonb_build_object(
            'ran', false,
