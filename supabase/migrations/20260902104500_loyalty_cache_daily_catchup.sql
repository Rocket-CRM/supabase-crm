-- Daily catch-up for custom loyalty caches (plan: LOYALTY_PROGRESS_EXPIRY_CACHE_PLAN.md).
-- Extends fn_loyalty_cache_refresh_5m: after each 5-minute dirty pass, process one
-- bounded backfill / straggler chunk per custom merchant (no new Render cron).
...
DECLARE
  ...
  v_catchup_batch_size constant integer := 500;
BEGIN
      ...
      BEGIN
        ...
        IF cardinality(v_users) > 0 THEN
          ...
          WHILE v_i <= cardinality(v_users) LOOP
            v_chunk := v_users[v_i : LEAST(v_i + 499, cardinality(v_users))];
        ...
        v_catchup := public.fn_loyalty_cache_catchup_chunk(
          ...
          v_as_of,
          v_catchup_batch_size
        );
        v_catchup_total := v_catchup_total + COALESCE((v_catchup->>'users_refreshed')::int, 0);
