-- Lifecycle automations: optional destination-tier filter for tier_upgrade / tier_downgrade.
-- Adds to_tier_id to workflow_trigger.trigger_conditions (matched by amp-dispatch-realtime-event).
...
BEGIN
  ...
  ELSE
    ...
    DELETE FROM workflow_node WHERE workflow_id = v_workflow_id AND merchant_id = v_merchant_id;
  ...
  LOOP
    ...
    v_node_config := COALESCE(v_action->'node_config', v_action) - 'id' - 'node_name';
