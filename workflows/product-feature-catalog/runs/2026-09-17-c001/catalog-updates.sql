BEGIN;
-- Weekly catalog maintenance run_id d6e2a91b-4c70-4f18-9e33-20260917c001
-- REVIEW ONLY — do not apply without human approval (no Supabase MCP on this Automation).
-- Scope:
--   1) Material update: loyalty.forms.surveys (Forms.md banner_url + completion rewards;
--      still unapplied from 2026-08-31 / 2026-09-07 / 2026-09-14 reviewable SQL)
--   2) Path-churn verifies (copy unchanged): Reward.md campaign reward relation layer;
--      Referral.md notifications outbox
--   3) New this week: platform.signup.login_methods verify after issueMemberSession
--      (Authentication.md + Signup_Login.md). Copy unchanged. JWT access-token TTL
--      is conflict-stopped (30d in JWT Generation vs 24h in Token Expiry / Signup_Login)
--      and is NOT written into summary/includes.
--   4) Re-carry evidence-backed verify/repair set from unapplied prior weekly SQL
--   5) No mutations for Shopify shopify_customer_id JWT work (code-only; sellable
--      storefront claims unchanged) or Customer 360 wallet-lots (conflict)
...
INSERT INTO _catalog_run VALUES ('d6e2a91b-4c70-4f18-9e33-20260917c001');
...
WHERE f.feature_key IN (
  ...
  'loyalty.reward.dynamic_pricing',
  'loyalty.reward.promo_codes',
  'loyalty.reward.groups_limits',
  'loyalty.reward.flash',
  'loyalty.reward.admin_push_claim',
...
UPDATE internal_product_feature f SET
  ...
  source_refs = '[
    {"kind":"knowledge","path":"requirements/Forms.md","heading":"Custom Fields System > Table Structure","query":"banner_url optional survey header"},
    {"kind":"requirements","path":"requirements/Forms.md","heading":"Custom Fields System > Table Structure","note":"banner_url optional survey header; published non-USER_PROFILE templates act as surveys"},
    {"kind":"requirements","path":"requirements/Forms.md","heading":"Custom Fields System > USER_PROFILE Form","note":"contrast: USER_PROFILE is profile/custom fields, not standalone survey"},
...
UPDATE internal_product_feature f SET
  source_refs = '[{"kind":"requirements","path":"requirements/Forms.md","heading":"Custom Fields System > USER_PROFILE Form"}]'::jsonb,
  last_verified_at = now(),
  updated_at = now()
...
-- verify (copy unchanged): loyalty.campaign.referral — Referral.md Notifications outbox is eng; sellable claim unchanged
UPDATE internal_product_feature f SET
  source_refs = '[
    {"kind":"requirements","path":"requirements/Referral.md","heading":"Core Referral Concept"},
    {"kind":"requirements","path":"requirements/Referral.md","heading":"Notifications (outbox)","note":"2026-09-04 churn: emit-only referral events; not a new sellable capability"}
  ]'::jsonb,
  last_verified_at = now(),
  updated_at = now()
WHERE f.feature_key = 'loyalty.campaign.referral';

-- verify (copy unchanged): loyalty.reward.catalog — Reward.md campaign slot layer is eng attach; visibility already GA
UPDATE internal_product_feature f SET
  source_refs = '[
    {"kind":"requirements","path":"requirements/Reward.md","heading":"Reward Properties","note":"visibility user/admin/campaign; campaign never in member catalog"},
    {"kind":"requirements","path":"requirements/Reward.md","heading":"Campaign reward relation layer","note":"2026-09-05 churn: slot attach/detach for referral/tier_entry/lifecycle — eng, not new row"}
  ]'::jsonb,
  last_verified_at = now(),
  updated_at = now()
WHERE f.feature_key = 'loyalty.reward.catalog';

-- verify (copy unchanged): loyalty.reward.eligibility
UPDATE internal_product_feature f SET
  source_refs = '[{"kind":"requirements","path":"requirements/Reward.md","heading":"Eligibility Dimensions"}]'::jsonb,
  last_verified_at = now(),
  updated_at = now()
WHERE f.feature_key = 'loyalty.reward.eligibility';

-- verify (copy unchanged): loyalty.reward.dynamic_pricing
UPDATE internal_product_feature f SET
  source_refs = '[{"kind":"requirements","path":"requirements/Reward.md","heading":"Dynamic Points Calculation"}]'::jsonb,
  last_verified_at = now(),
  updated_at = now()
WHERE f.feature_key = 'loyalty.reward.dynamic_pricing';

-- verify (copy unchanged): loyalty.reward.promo_codes
UPDATE internal_product_feature f SET
  source_refs = '[{"kind":"requirements","path":"requirements/Reward.md","heading":"Promo Code Management"}]'::jsonb,
  last_verified_at = now(),
  updated_at = now()
WHERE f.feature_key = 'loyalty.reward.promo_codes';

-- verify (copy unchanged): loyalty.reward.groups_limits
UPDATE internal_product_feature f SET
  source_refs = '[{"kind":"requirements","path":"requirements/Reward.md","heading":"Redemption Limits"}]'::jsonb,
  last_verified_at = now(),
  updated_at = now()
WHERE f.feature_key = 'loyalty.reward.groups_limits';

-- verify (copy unchanged): loyalty.reward.flash
UPDATE internal_product_feature f SET
  source_refs = '[{"kind":"requirements","path":"requirements/Reward.md","heading":"Reward Properties","note":"relative_mins expiration mode"}]'::jsonb,
  last_verified_at = now(),
  updated_at = now()
WHERE f.feature_key = 'loyalty.reward.flash';

-- verify (copy unchanged): loyalty.reward.admin_push_claim
UPDATE internal_product_feature f SET
  source_refs = '[{"kind":"requirements","path":"requirements/Reward.md","heading":"Executive Summary"}]'::jsonb,
  last_verified_at = now(),
  updated_at = now()
WHERE f.feature_key = 'loyalty.reward.admin_push_claim';

-- verify (copy unchanged): platform.signup.login_methods
-- 2026-09-14 issueMemberSession shared issuer is implementation. Sellable claim stays LINE / phone OTP.
-- Do not encode access-token TTL: Authentication JWT Generation says 30 days; Token Expiry + Signup_Login still say 24 hours.
UPDATE internal_product_feature f SET
  source_refs = '[
    {"kind":"knowledge","path":"requirements/Signup_Login.md","heading":"Authentication Methods Configuration","query":"LINE phone OTP auth_methods"},
    {"kind":"requirements","path":"requirements/Signup_Login.md","heading":"Authentication Methods Configuration"},
    {"kind":"knowledge","path":"requirements/Authentication.md","heading":"End-User Authentication > JWT Generation (Custom Claims)","query":"issueMemberSession"},
    {"kind":"requirements","path":"requirements/Authentication.md","heading":"End-User Authentication > JWT Generation (Custom Claims)","note":"shared issuer across LINE/phone/Shopify; TTL 30d here vs 24h in Token Expiry — not a sales claim"}
  ]'::jsonb,
  last_verified_at = now(),
  updated_at = now()
...
-- provenance repair only (Admin_Panel.md missing from this repo; registry pointer — do not bump last_verified_at)
UPDATE internal_product_feature f SET
