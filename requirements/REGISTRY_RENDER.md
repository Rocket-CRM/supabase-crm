...
C: daily-currency-expiry — `0 19 * * *` → `CALL process_expiry_if_needed()` **[RETIRED 2026-09-02 — Render `currency-expiry-daily`]**
...
C: loyalty-cache-refresh — Render Cron Job `*/5 * * * *` → `fn_loyalty_cache_refresh_5m()` (custom tier/expiry cache; dirty users only)
C: loyalty-cache-catchup-daily — Render Cron Job `0 19 * * *` UTC → `fn_loyalty_cache_catchup_chunk` loop (5000-user unfinished backfill)
...
R: loyalty-cache-refresh — Render Cron Job (`*/5 * * * *`); one-shot `npm run job:loyalty-cache-refresh` on `crm-event-processors` image; calls `fn_loyalty_cache_refresh_5m` (dirty custom cache only).
R: loyalty-cache-catchup-daily — Render Cron Job (`0 19 * * *` UTC ≈ 02:00 ICT); one-shot `npm run job:loyalty-cache-catchup-daily` on `crm-event-processors` image; loops `fn_loyalty_cache_catchup_chunk` (5000 users per RPC) over `SUPABASE_DB_DIRECT_URL` until unfinished backlog is empty.
