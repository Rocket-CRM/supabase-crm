...
- **Demo bootstrap missions** — `custom_ausiris_demo_bootstrap_engagement` still inserts some standard missions with `allow_progress_loop = false` and no progress-limit rows; re-running that seed without updating it can create repeatable missions. Prefer admin or AMP draft for new missions.
